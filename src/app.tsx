...
const spellBuildups: Record<SpellName, number> = {
		"Heal":					1,
		"BigFireBall": 	4,
		"Fireball": 		2,
		"Reflect": 			3,
...
export function App() {
	const [p1Buildups, setP1Buildups] = useState(spellBuildups)
	const [p2Buildups, setP2Buildups] = useState(spellBuildups)
	...
	function initMoveSet(isP1: boolean): Record<SpellName, Move> {
		...
		for(const index in activeSpells){
			...
			moveSet[spellName] = {
				...
				cooldown: spellBuildups[spellName],
	...
	async function cooldownSpell(isP1: boolean, name: SpellName){
		...
		const cooldown = spellBuildups[name]
						...
						{Array.from({length: player1.hp}).map(() => (
							...
							{Object.entries(p1Moves.current).map(([key, value], _) => {
								...
								return (
									<Buildup
										key={key}
										disabled={p1Disabled[k]}
										lastTriggered={p1Triggered[k]}
										buttonDown={p1DownKeys[k]}
										buildup={p1Buildups[k]}
										glowColor='red'
										glowRadiusFactor={10}
